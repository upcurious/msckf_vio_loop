...
struct Feature {
  ...
  std::map<StateIDType, std::pair<Eigen::Vector4d,Eigen::Vector4d>, std::less<StateIDType>,
    Eigen::aligned_allocator<
      std::pair<const StateIDType, std::pair<Eigen::Vector4d,Eigen::Vector4d>> > > observations;
...
bool Feature::checkMotion(
  ...
  Eigen::Vector3d feature_direction(
      observations.begin()->second.first(0),
      observations.begin()->second.first(1), 1.0);
...
bool Feature::initializePosition(
  ...
  for (auto& m : observations) {
    ...
    // Add the measurement.
    measurements.push_back(m.second.first.head<2>());
    measurements.push_back(m.second.first.tail<2>());
