...
void ImageProcessor::publish() {
  ...
  for (int i = 0; i < curr_ids.size(); ++i) {
    ...
    feature_msg_ptr->features[i].u0_o = curr_cam0_points[i].x;
    feature_msg_ptr->features[i].v0_o = curr_cam0_points[i].y;
    feature_msg_ptr->features[i].u1_o = curr_cam1_points[i].x;
    feature_msg_ptr->features[i].v1_o = curr_cam1_points[i].y;
  }
