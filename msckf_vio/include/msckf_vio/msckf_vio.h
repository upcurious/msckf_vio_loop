...
class MsckfVio {
  ...
  private:
    ...
    void setReloFrame(double _frame_stamp, int _frame_index, vector<pair<Vector3d,Vector3d>> _match_points,Vector3d _relo_t, Matrix3d _relo_r);
    void relofeatureJacobian(const pair<Vector3d,Vector3d>& feature_id,
        const std::vector<StateIDType>& cam_state_ids,
        Eigen::MatrixXd& H_x, Eigen::VectorXd& r);
    void relomeasurementUpdate(const Eigen::MatrixXd& H,
        const Eigen::VectorXd& r);
    void relomeasurementJacobian(const StateIDType& cam_state_id,
