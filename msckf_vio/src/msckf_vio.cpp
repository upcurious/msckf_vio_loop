...
//  std::cout<<msg->header.stamp.toSec()<<std::endl;
  while (!relo_buf.empty())
  {
      ...
      relo_buf.pop();
//      if(abs(relo_msg->header.stamp.toSec() - msg->header.stamp.toSec()) < min)
...
//  cout<<"====="<<endl;
  ...
  if (relo_msg != NULL)
  {
      vector<pair<Vector3d,Vector3d>> match_points;
      double frame_stamp = relo_msg->header.stamp.toSec();
      for (unsigned int i = 0; i < relo_msg->points.size()/2; i++)
      {
	  Vector3d u_v_id,point_3d;
	  u_v_id.x() = relo_msg->points[i].x;
	  u_v_id.y() = relo_msg->points[i].y;
	  u_v_id.z() = relo_msg->points[i].z;
	  
	  point_3d.x() = relo_msg->points[i*2+1].x;
	  point_3d.y() = relo_msg->points[i*2+1].y;
	  point_3d.z() = relo_msg->points[i*2+1].z;
	  match_points.push_back(make_pair(u_v_id,point_3d));
      }
  ...
  for (const auto& feature : msg->features) {
    if (map_server.find(feature.id) == map_server.end()) {
      ...
      map_server[feature.id].observations[state_id] =
        make_pair(Vector4d(feature.u0, feature.v0,
            feature.u1, feature.v1),Vector4d(feature.u0_o, feature.v0_o,
            feature.u1_o, feature.v1_o));
    } else {
...
void MsckfVio::measurementJacobian(
  ...
  const Vector4d& z = feature.observations.find(cam_state_id)->second.first;
...
void MsckfVio::publish(const ros::Time& time) {
  ...
  if(marginalization_flag == MARGIN_OLD)
    pub_keyframe_pose.publish(odom_msg);
  ...
  point_cloud.header = odom_msg.header;
  ...
  for (const auto& item : map_server) {
    ...
    if (feature.is_initialized) {
	  ...
	  p_2d.values.push_back(feature.observations.find(state_server.imu_state.id)->second.second(0));
	  p_2d.values.push_back(feature.observations.find(state_server.imu_state.id)->second.second(1));
	  p_2d.values.push_back(feature.observations.find(state_server.imu_state.id)->second.first(0));
	  p_2d.values.push_back(feature.observations.find(state_server.imu_state.id)->second.first(1));
	  p_2d.values.push_back(feature.observations.find(state_server.imu_state.id)->second.first(2));
	  p_2d.values.push_back(feature.observations.find(state_server.imu_state.id)->second.first(3));
	  p_2d.values.push_back(feature.id);
	  point_cloud.channels.push_back(p_2d);
// 	  cout<<p<<endl;
// 	  cout<<feature.observations.find(state_server.imu_state.id)->second.first<<endl;
// 	  cout<<"==="<<endl;
      }
    }
  }
  ...
  const Matrix3d& R_i_c = state_server.imu_state.R_imu_cam0;
  const Vector3d& t_c_i = state_server.imu_state.t_cam0_imu;
  Matrix3d R_c_i = R_i_c.transpose();
//   Vector3d t_c_i_ = -R_c_i*t_c_i;
  odometry.pose.pose.position.x = t_c_i(0);
  odometry.pose.pose.position.y = t_c_i(1);
  odometry.pose.pose.position.z = t_c_i(2);
  Quaterniond tmp_q{R_c_i};
...
#if 1
void MsckfVio::setReloFrame(double _frame_stamp, int _frame_index, vector<pair<Vector3d,Vector3d>> match_points,Vector3d _relo_t, Matrix3d _relo_r)
{

  
  int jacobian_row_size = 0;

  for (auto &iter : match_points) {
     auto cam_state_iter = map_server.find(iter.first(2));
    if (cam_state_iter == map_server.end()) continue;
    auto& feature = map_server[iter.first(2)];
  ...
  // Process the features which lose track.
  for (const auto& feature_id : match_points) {
    auto cam_state_iter = map_server.find(feature_id.first(2));
    if (cam_state_iter == map_server.end()) continue;
    auto& feature = map_server[feature_id.first(2)];
    vector<StateIDType> cam_state_ids(0);
    for (const auto& measurement : feature.observations)
      cam_state_ids.push_back(measurement.first);
    if (cam_state_ids.size() == 0) continue;
    MatrixXd H_xj;
    VectorXd r_j;
    relofeatureJacobian(feature_id, cam_state_ids, H_xj, r_j);
    if (gatingTest(H_xj, r_j, cam_state_ids.size()-1)) {
      H_x.block(stack_cntr, 0, H_xj.rows(), H_xj.cols()) = H_xj;
      r.segment(stack_cntr, r_j.rows()) = r_j;
      stack_cntr += H_xj.rows();
...
void MsckfVio::relofeatureJacobian(
    const pair<Vector3d,Vector3d>& feature_id,
    const std::vector<StateIDType>& cam_state_ids,
    MatrixXd& H_x, VectorXd& r) {

  const auto& feature = map_server[feature_id.first(2)];
  ...
  for (const auto& cam_id : valid_cam_state_ids) {
    ...
    relomeasurementJacobian(cam_id, feature_id, H_xi, H_fi, r_i);
...
void MsckfVio::relomeasurementJacobian(
    const StateIDType& cam_state_id,
    const pair<Vector3d,Vector3d>& feature_id,
  ...
  // Prepare all the required data.
  const CAMState& cam_state = state_server.cam_states[cam_state_id];
//   const Feature& feature = map_server[feature_id(2)];
  ...
  // 3d feature position in the world frame.
  // And its observation with the stereo cameras.
  const Vector3d& p_w = feature_id.second;
  const Vector2d& z = Vector2d(feature_id.first(0),feature_id.first(1));
